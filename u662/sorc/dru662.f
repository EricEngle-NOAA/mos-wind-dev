...
C                                ADAPTED FROM DRU660; INDEX( ,)
C                                TWO DIMENSIONAL
C
C        PURPOSE
C           DRIVER FOR PROGRAM U662.  SEE U662 COMMENTS OR PROGRAM
C           WRITEUP FOR VARIABLE DEFINITIONS.  AN ATTEMPT HAS BEEN
C           MADE TO INCLUDE ALL INFORMATION IN THIS DRIVER THAT
C           THE USER OF U662 MIGHT HAVE TO CHANGE.
C
C        DATA SET USE
...
C
     ...
     1     KFILDO/12/
      ...
      DATA NWHERE/ND4*0/
C
      CALL TIMPR(KFILDO,KFILDO,'START U662          ')
C
      CALL U662(KFILDI,KFILDO,
     1          CCALL,ISDATA,SDATA,NGP,XDATA,MWORK,NAME,ND1,
     2          ID,IDPARS,TRESHL,TRESHU,JD,INDEX,JP,
     3          ITAU,IWDTH,WDTH,IPREC,PREC,CFMT,NWHERE,NWRITE,
     ...
     C          CORE,ND10,NBLOCK)
C
      CALL TIMPR(KFILDO,KFILDO,'END U662            ')
      STOP 662
      END
