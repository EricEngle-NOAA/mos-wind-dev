      PROGRAM DRU830
C
C        OCTOBER  1999   GLAHN   TDL   MOS-2000
C        AUGUST   2000   DALLAVALLE    CORRECTED THE DIMENSION OF
C                                      ARRAY MSTORE.
C
C        PURPOSE
C           DRIVER FOR PROGRAM U830.  SEE U830 COMMENTS OR PROGRAM
C           WRITEUP FOR VARIABLE DEFINITIONS.  AN ATTEMPT HAS BEEN
C           MADE TO INCLUDE ALL INFORMATION IN THIS DRIVER THAT
C           THE USER OF U830 MIGHT HAVE TO CHANGE.
C
C        DATA SET USE
...
C                   IN U830.  THIS DOES NOT HAVE TO BE LARGE ENOUGH TO HOLD 
C                   ALL THE STATIONS ON THE INPUT FILES.
...
C
     ...
     1     KFILDO/12/
      ...
      DATA NWHERE/ND4*0/
C
      CALL TIMPR(KFILDO,KFILDO,'START U830          ')
C
      CALL U830(KFILDI,KFILDO,
     ...
     C          CORE,ND10,NBLOCK)
C
      CALL TIMPR(KFILDO,KFILDO,'END U830            ')
      STOP 830
      END
