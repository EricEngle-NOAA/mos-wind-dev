...
C                                 TTOTALS,SWEATI,DIR2UV,OBSDMAXT,OBSNMINT,
...
C        SEPT      2005   RLC     FOR QUARTERLY LIBRARY UPDATES IT WAS DECIDED
C                                 NOT TO PUT CALLS TO U202 GRIDDED POST-
C                                 PROCESSING ROUTINES INTO OPTN2 SINCE THEY
C                                 ARE JUST OPERATIONAL POST-PROCESSING.  SO I
C                                 REMOVED THE CALLS FROM 4/2005 TO SHFTMXMN,
C                                 CKTMPDP, AND FTOKGRD.
...
C                                 VERSION BY ADDING PREVIOUS WSPEED CHANGE AND
C                                 CALL TO GUSTRAT.  MOVED CALLS TO FORIER AND 
C                                 KINXRF INTO PROPER ORDER BY CCC.
C        MAR       2007   RLC     CORRECTED CALL TO BULKRN
C        FEB       2008   RLC     ADDED CALL TO OBSQCLD.
C        SEPT      2008   ANTOLIK ADDED CALL TO PCPEFF.  OTHER MODS TO
C                                 ENHANCE CONSISTENCY WITH OPTION AND
C                                 WITH OPERATIONAL VERSION.
C        OCT       2008   ANTOLIK ADDED CALLS TO ADJUSTED WIND OBS ROUTINES 
C                                 FOR BCDG ANALYSIS.
C        SEPT      2009   VEENHUI MODIFIED CALLS TO OBSDMAXT AND OBSNMINT TO 
C                                 INCLUDE PASSING THE ARRAY NAME
C        MAR       2010   ANTOLIK COMMENTED OUT SECTION DEALING WITH CONSTANTS.
C                                 REVISED COMMENTS TO MORE THOROUGHLY EXPLAIN
C                                 OPTION CALLS OMITTED FROM THIS ROUTINE.
C        NOV       2010   ANTOLIK UPDATED SWTXRF CALL FOR NEW VERSION COMPATIBLE
C                                 WITH SCHEME TWO RELATIVE FREQUENCIES.
C        AUG       2011   ANTOLIK ADDED KINXRF2 CALL FOR 2H CONVECTION.   
C        AUG       2012   SHAFER  ADDED IDS FOR 2H TSTORMS TO KINXRF2 CALL.
C        DEC       2014   ENGLE   ADDED NSLAB TO CALL TO GUSTRAT.
C
C        PURPOSE 
...
C            MORE MODERN FORTRAN COMPILERS, BUT IT IS STILL MORE
...
C            ARE:
C               TIMEP,L1D,L2D,
C            THE ROUTINES DEALING WITH CONSTANT DATA:
C               CONST,CONST1,GRDCNST,GRDCNST_MRG,
C            THE TEMPORAL INTERPOLATION ROUTINES:
C               TIMTRP,TGINTRP,
C            AND THE LOGIT TRANSFORM ROUTINE:
C               LOGITTRANS.
C            THE LAST-DITCH CALL TO AVHRLY IS ALSO OMITTED.
C            IN ADDITION, A 2005 DECISION WAS MADE TO EXCLUDE ANY GRID
C            POST-PROCESSING ROUTINES.
C
C        DATA SET USE 
...
C            SWEATI, DIR2UV,   OBSDMAXT, OBSNMINT, PCPX6,  SVRVEC,
C            KINXRF, KINXRF2, RHVV, FRZLVL, SSR, ZRPRED,
...
C        COMPUTED.  IDPARS(10) IS NOT CHECKED IN OPTN2; 
C        WHEN BOTH VERTICAL COMPUTATION AND TIME COMPUTATION 
C        IS DONE, TIME IS ENTERED FIRST (IN OPTION) SO
C        THAT VERTICAL COMPUTATION IS DONE WITHIN IT (IN OPTN2).
C
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
CC        LOOK FOR CONSTANT DATA, TO BE PROVIDED IN
CC        THE MOS-2000 EXTERNAL RANDOM ACCESS FILES.
CC        THIS SECTION SHOULD NOT BE NEEDED BY OPTN2, 
CC        SO IT IS COMMENTED OUT IN THAT ROUTINE
CC
CC       
C      ELSEIF(IDPARS(1).GE.400.AND.IDPARS(1).LE.499)THEN
CC
CC        LOOK FOR COMPUTATION OF THE FOLLOWING GRIDDED CONSTANTS: 
CC        (1) NORMAL MAX OR NORMAL MIN, OR
CC        (2) GRIDDED SNOWFALL CLIMATOLOGY, OR
CC        (3) GRIDDED PRECIP TYPE RELATIVE FREQUENCIES, OR
CC        (4) GRIDDED PRECIP TYPE 50% VALUES
CC
C      IF(((IDPARS(1).EQ.432).AND.((IDPARS(2).EQ.540).OR.
C     1                            (IDPARS(2).EQ.541))).OR.
C     2   ((IDPARS(1).EQ.422).AND.((IDPARS(2).EQ.540).OR.
C     3                            (IDPARS(2).EQ.541))).OR.
C     4   ((IDPARS(1).EQ.438).AND.((IDPARS(2).EQ.344).OR.
C     5                            (IDPARS(2).EQ.544).OR.
C     6                            (IDPARS(2).EQ.545))).OR.
C     7   ((IDPARS(1).EQ.461).AND.((IDPARS(2).EQ.541).OR.
C     8                            (IDPARS(2).EQ.542))).OR.
C     9   ((IDPARS(1).EQ.462).AND.((IDPARS(2).EQ.541).OR.
C     A                            (IDPARS(2).EQ.542).OR.
C     B                            (IDPARS(2).EQ.543))))THEN
CC
CC        IF DD=99 THEN GRDCNST_MRG IS CALLED TO MERGE DATA READ
CC        FROM TWO OR MORE GRIDDED RANDOM ACCESS FILES.
CC        THIS IS FOR GRIDDED DATA AND WAS WRITTEN FOR U202.
CC     
C         IF(IDPARS(4).EQ.99) THEN
C            CALL GRDCNST_MRG(KFILDO,IDPARS,ID,JD,NDATE,KFILRA,RACESS,
C     1                    NUMRA,CCALL,NAME,STALAT,STALON,DIR,ND1,
C     2                    NSTA,NGRIDC,NGRID,ND11,NSLAB,IPACK,IWORK,
C     3                    DATA,ND5,IS0,IS1,IS2,IS4,ND7,ND2X3,FD1,
C     4                    IP12,ISTAV,L3264B,IER)
C         ELSE
CC
CC        IF DD IS ANYTHING OTHER THAN 99 THEN INTERPOLATE 
CC        GRIDDED CONSTANT TO STATIONS:
CC        
C            CALL GRDCNST(KFILDO,IDPARS,ID,JD,NDATE,
C     1                   KFILRA,RACESS,NUMRA,CCALL,
C     2                   NAME,STALAT,STALON,SDATA,
C     3                   DIR,ND1,NSTA,NGRIDC,NGRID,ND11,NSLAB,
C     4                   IPACK,IWORK,DATA,ND5,
C     5                   IS0,IS1,IS2,IS4,ND7,FD1,FD2,ND2X3,
C     6                   IP12,ISTAV,L3264B,IER)
C         ENDIF
CC
C      ELSE
CC
CC        CONSTANT VARIABLES NOT SATISFYING ANY OF THE ABOVE
CC        CRITERIA (I.E. AT STATIONS) ARE PROCESSED THROUGH CONST1:
CC
C            CALL CONST1(KFILDO,KFIL10,IP12,
C     1                 ID,IDPARS,JD,NDATE,
C     2                 KFILRA,RACESS,NUMRA,
C     3                 CCALL,ICALLD,CCALLD,NAME,STALAT,STALON,
C     4                 ISDATA,SDATA,DIR,ND1,NSTA,
C     5                 NGRIDC,NGRID,ND11,NSLAB,
C     6                 IPACK,IWORK,DATA,ND5,
C     7                 LSTORE,ND9,LITEMS,CORE,ND10,LASTL,
...
C        VERSION HAS BEEN REDONE TO CORRECT ARGUMENT INCONSISTENCY
C        AND ADD ID SCHEME 2 COMPATIBILITY. REPLACES CALL
C        PREVIOUSLY COMMENTED OUT BY RLC.
C
      ELSEIF((IDPARS(1).EQ.007).AND.((IDPARS(2).GE.361)
     1                         .AND.(IDPARS(2).LE.391)))THEN
         CALL SWTXRF(KFILDO,KFIL10,IP12,IP16,IDPARS,JD,NDATE,
     1               KFILRA,RACESS,NUMRA,
     2               CCALL,ICALLD,CCALLD,NAME,STALAT,STALON,
     3               ISDATA,SDATA,DIR,ND1,NSTA,LASTL,LASTD,
     4               NGRID,NGRIDC,ND11,NSLAB,IPACK,IWORK,DATA,ND5,
     5               LSTORE,ND9,LITEMS,CORE,ND10,
     6               NBLOCK,NFETCH,NSTORE,
     7               IS0,IS1,IS2,IS4,ND7,
     8               FD1,FD2,FD3,FD4,FD5,FD6,FD7,ND2X3,
     9               ISTAV,L3264B,L3264W,MISTOT,IER)
C
...
C
     ...
     5             ISTAV,L3264B,MISTOT,IER)
C
C        LOOK FOR COMPUTATION OF KF PREDICTOR FOR 2-H CONVECTION
C        AND 2-H THUNDERSTORMS
C
      ELSEIF(IDPARS(1).EQ.007.AND.(IDPARS(2).EQ.451.OR.
     1                             IDPARS(2).EQ.456.OR.
     2                             IDPARS(2).EQ.452.OR.
     3                             IDPARS(2).EQ.457))THEN
         CALL KINXRF2(KFILDO,KFIL10,IP12,IP16,IDPARS,JD,NDATE,
     1               KFILRA,RACESS,NUMRA,
...
C
      ELSEIF((IDPARS(1).EQ.702).AND.(IDPARS(2).EQ.1))THEN
         CALL OBSDMAXT(KFILDO,KFIL10,ID,IDPARS,JD,NDATE,NAME,
...
C
      ELSEIF(IDPARS(1).EQ.702.AND.IDPARS(2).EQ.011)THEN
         CALL OBSNMINT(KFILDO,KFIL10,ID,IDPARS,JD,NDATE,NAME,
     1                 ITIMEZ,SDATA,ND1,NSTA,
     2                 CCALL,IPACK,IWORK,FD1,FD2,ND2X3,
     3                 LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     4                 IS0,IS1,IS2,IS4,ND7,
...
C
      ELSEIF(IDPARS(1).EQ.704.AND.(IDPARS(2).EQ.020).OR.
     1                            (IDPARS(2).EQ.120)) THEN
         CALL OBSADJUV(KFILDO,KFIL10,IDPARS,JD,NDATE,
...
C
      ELSEIF(IDPARS(1).EQ.704.AND.IDPARS(2).EQ.230) THEN
         CALL OBSADJDIR(KFILDO,KFIL10,IDPARS,JD,NDATE,
     1                  SDATA,ND1,NSTA,IPACK,IWORK,FD1,FD2,ND2X3,
     2                  LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3                  IS0,IS1,IS2,IS4,ND7,
     4                  ISTAV,L3264B,IER)
...
C
      ELSEIF(IDPARS(1).EQ.704.AND.IDPARS(2).EQ.330)THEN
         CALL OBSADJSPD(KFILDO,KFIL10,IP12,
     1                  IDPARS,JD,NDATE,
     2                  KFILRA,RACESS,NUMRA,CCALL,ICALLD,
     3                  CCALLD,ISDATA,SDATA,ND1,NSTA,FD1,ND2X3,
     4                  NSLAB,IPACK,IWORK,
     5                  DATA,ND5,LSTORE,ND9,LITEMS,CORE,ND10,LASTL,
...
C        VARIABLES.
C
      ELSEIF(IDPARS(1).EQ.708.AND.IDPARS(2).EQ.313)THEN
         CALL OBSQCLD(KFILDO,KFIL10,ID,IDPARS,JD,NDATE,
     1                CCALL,STALAT,STALON,ISDATA,SDATA,ND1,NSTA,
