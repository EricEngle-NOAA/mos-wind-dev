...
C                                 INCLUDE PASSING OF THE ARRAY "NAME".
C        JAN       2010   ANTOLIK UPDATED CALL TO TGINTRP TO INCLUDE I=8.
C                                 REORGANIZED SECTION FOR CONSTANT VALUES,
C                                 ADDED CALL TO GRDCNST_MRG.
C        MAY       2010   ANTOLIK MADE ALL NECESSARY MODIFICATIONS TO ADD U202
C                                 GRID POST-PROCESSING ROUTINES.  THESE WERE 
C                                 ORIGINALLY ADDED TO OPERATIONAL VERSION 
C                                 BETWEEN AUG 2005 AND JUNE 2007 BY RLC/JCM.
C                                 ADDED CALL TO NEW ROUTINE GRDUNTCVT WHICH DOES
C                                 VARIOUS UNIT CONVERSIONS FOR GMOS.  THIS
C                                 ROUTINE REPLACED FTOKGRD, SO THAT CALL WAS
C                                 REMOVED.  UPDATED CALL TO CKBOUNDS. ADDED
C                                 NEW CALLS TO CMPWDRGR, CKWSPDGR, CKWNDGST,
C                                 CKGPOP, AND CMPQPFGR.
C        NOV       2010   ANTOLIK UPDATED SWTXRF CALL FOR NEW VERSION COMPATIBLE
C                                 WITH SCHEME TWO RELATIVE FREQUENCIES.
C        AUG       2011   ANTOLIK ADDED KINXRF2 CALL FOR 2H CONVECTION AND
C                                 REQUIRED BYPASS FOR TIMTRP CALL.
C        AUG       2012   SHAFER  ADDED IDS FOR 2H TSTORMS TO KINXRF2 CALLS.
...
C            OBSDMAXT, OBSNMINT, PCPX6, SVRVEC, KINXRF, KINXRF2,
C            OBSTCLD, OBSCIGHT, OBSPTYPE, OTSTCLD, SFCTCLD,
C            CORDP, SNOWFL, RHVV, FRZLVL, SSR, ZRPRED,
...
C            HELCTY, BULKRN, RHNSPD, ADJTMP, SHFTMXMN, WINDSP, CKTMPDP,
C            CKBOUNDS, CMPDPGR, CMPRHGR, GRDUNTCVT, CMPWDRGR, CKWSPDGR,
C            CKWNDGST, CKGPOP, CMPQPFGR, OBSGUST, GUSTRAT, GRDCNST,
C            GRDCNST_MRG, LOGITTRANS, OBSQCLD, PCPEFF.
C
...
C
C        AN EXCEPTION HAS BEEN ADDED HERE FOR THE KF INTERACTIVE
C        PREDICTOR FOR 2-H TSTORMS/CONVECTION. TIME INTERPOLATION OF
C        THE K-INDEX IS HANDLED FROM WITHIN ROUTINE KINXRF2, SO TIMTRP
C        SHOULD NOT BE ENTERED FOR THIS VARIABLE. KINXRF2 IS CALLED
C        DIRECTLY INSTEAD:
C
C        LOOK FOR COMPUTATION OF KF PREDICTOR FOR 2-H TSTORMS/CONVECTION
C
         IF(IDPARS(1).EQ.007.AND.(IDPARS(2).EQ.451.OR.
...
C     
         IF(IDPARS(4).EQ.99) THEN
            CALL GRDCNST_MRG(KFILDO,IDPARS,ID,JD,NDATE,KFILRA,RACESS,
     1                    NUMRA,CCALL,NAME,STALAT,STALON,DIR,ND1,
     2                    NSTA,NGRIDC,NGRID,ND11,NSLAB,IPACK,IWORK,
     3                    DATA,ND5,IS0,IS1,IS2,IS4,ND7,ND2X3,FD1,
     4                    IP12,ISTAV,L3264B,IER)
         ELSE
C
...
C
     ...
     +                               (IDPARS(2).EQ.354).OR.
     B                               (IDPARS(2).EQ.374).OR.
     C                               (IDPARS(2).EQ.384).OR.
     D                               (IDPARS(2).EQ.044))).OR.
     E        (IDPARS(1).EQ.224.AND.((IDPARS(2).EQ.374).OR.
     F                               (IDPARS(2).EQ.394))).OR.
     G        (IDPARS(1).EQ.227.AND.((IDPARS(2).EQ.124).OR.
     H                               (IDPARS(2).EQ.224).OR.
     I                               (IDPARS(2).EQ.234).OR.
     J                               (IDPARS(2).EQ.334).OR.
     K                               (IDPARS(2).EQ.434)).OR.
     L         (IDPARS(1).EQ.228.AND.((IDPARS(2).EQ.399).OR.
     M                               (IDPARS(2).EQ.474)))))THEN
...
C
     ...
     A               ISTAV,L3264B,L3264W,IER)
C
C       LOOK FOR CHECK OF CONSISTENT AND BOUNDED 12-H GRIDDED POP AGAINST
